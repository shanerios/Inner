...
/** Every designed world's binaural layer breathes: 4 s in, 8 s out, each breath up to 15% different, easing as sleep deepens. */
const BINAURAL_BREATH: WorldBinaural['breath'] = {
  depthDb: { preparation: 10, descent: 10, earlySleep: 6, remSleep: 3 },
  inhaleSeconds: 4,
  exhaleSeconds: 8,
  variation: 0.15,
};
...
const SHARED_BINAURAL: WorldBinaural = {
  ...
  trimDb: 0,
  levelDb: 0,
  breath: { depthDb: { preparation: 0, descent: 0, earlySleep: 0, remSleep: 0 }, inhaleSeconds: 4, exhaleSeconds: 8, variation: 0 },
...
export type WorldBinaural = {
  ...
  levelDb: number;
  /**
   * The slow breath of the layer: its level rises over an inhale and eases back over an exhale, so it moves with the world
   * instead of holding still. The depth is how far it falls from the top of a breath, and eases as sleep deepens; each
   * breath is a little different (the variation), so it is never a metronome.
   */
  breath: {
    depthDb: { preparation: number; descent: number; earlySleep: number; remSleep: number };
 ...
 */
export type BinauralStageOverrides = {
...
export function binauralForWorld(
  ...
  return {
    ...
    binauralBreathDb: binaural.breath.depthDb.preparation,
    binauralBreathInSeconds: binaural.breath.inhaleSeconds,
    binauralBreathOutSeconds: binaural.breath.exhaleSeconds,
    binauralBreathVariation: binaural.breath.variation,
  };
}
